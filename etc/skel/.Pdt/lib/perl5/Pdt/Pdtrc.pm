...
# written and sourced frequently. .pdtrc 
# should also be sourced at the very end 
...
export PDT_ACTIVE=<TMPL_VAR NAME=PDT_ACTIVE> # project active directory
export PDT_SCM_DIR=$PDT_ROOT/$PDT_ACTIVE/Scm	# project local scm image
export PDT_GIT_DIR=$PDT_ROOT/$PDT_ACTIVE/Git/$PDT_ACTIVE	# the publishable image dir
export PDT_SCM_PATH=$PDT_SCM_DIR/$PDT_ACTIVE
export PDT_BIN_PATH=$PDT_SCM_PATH/bin
export PDT_LIB_PATH=$PDT_SCM_PATH/lib
export PDT_TMP_PATH=$HOME/.tmp
...
export PDT_PHOTO_PATH=$PDT_ROOT/$PDT_ACTIVE/Photo
export PDT_STILL_PATH=$PDT_ROOT/$PDT_ACTIVE/Photo
export PDT_DATA_PATH=$PDT_ROOT/$PDT_ACTIVE/Data
export PDT_EXE_PATH=$PDT_ROOT/$PDT_ACTIVE/Exe_Pub
...
export PDT_RESOURCE_PATH=$PDT_ROOT/$PDT_ACTIVE/Resource
...
# DEPRECATED. Git should not be assumed, and we should 
# take this out of the distro. This distro writes and 
# backs up code. Integrating with 3rd party SCM is out 
# of scope.
# 
...
if [ $PDT_GIT_ON == 1 ]
then
export PDT_GIT_PATH=$PDT_GIT_DIR
...
export PDT_GIT_REPO=$PDT_GIT_DIR

# export PDT_GIT_TOKEN=<CONFIGURE ME IN .xinitrc>

# PDT_GIT_TOKEN is used by project -u and setpdtrc -u for 
# running all the github sync commands automatically. 
# this is configured by setting up "classic token" 
# two factor auth on github
fi
...
export PERL5LIB="$PDT_LIB_PATH:$PDT_LIB:$PERL5LIB"		# Add the Pdt libs for current working project

# vim. We may want to use a destinct editor.
# this gives us the opportunity to define it.

export PDT_EDITOR=`/usr/bin/which vim`
...
# $PROMPT_COMMAND is defined in .bashrc, and 
# should stay there. (forgot why, but pretty sure) 
...
# vectored bin and lib directories. Note: this 
# needs to be updated and handled symlink style. 
...
alias idelib='eval `$PDT_BIN/cdidelib`'
alias pdtbin='eval `$PDT_BIN/cdpdtbin`'
alias idebin='eval `$PDT_BIN/cdidebin`'

alias pdtscm='eval `$PDT_BIN/cdpdtscm`'
...
alias pdtroot='eval `$PDT_BIN/cdpdtroot`'
