package Pdt::Form::Entry;    # (P: o)

# #
my $VERSION = '2018-04-13.07-04-51.EDT';

#: Entry Widget for Pdt Forms
...
sub cbmap { # (:C cbmap)
   ...
   $self->{'cbmap'}->{'charblank'}      = sub { shift; return $self->charblank(@_); };
...
# get a blank character block the length of the field

sub charblank { # (M: stacksuper)
   my $self = shift;
   return ( $self->SUPER::charblank(@_) );
}
